...
// From the original PolyGlot source by Fabien Letouzey (dual-LCG PRNG).
// Layout: [0..768] piece-square keys (piece_kind*2+pg_color, 64 squares each)
//         [768..772] castling keys (white OO, white OOO, black OO, black OOO)
//         [772..780] en passant file keys (a-h)
//         [780] turn key (XOR when white to move)
const polyglot_random = [781]u64{
...
pub const Book = struct {
    ...
    // Parse book entries from raw file bytes (big-endian, 16 bytes per entry).
    // The caller is responsible for freeing buf after this returns.
    pub fn fromBytes(allocator: std.mem.Allocator, buf: []const u8, seed: u64) !Book {
        if (buf.len % 16 != 0) return error.InvalidBookFile;
        const num_entries = buf.len / 16;

        const entries = try allocator.alloc(BookEntry, num_entries);
        for (0..num_entries) |i| {
            const off = i * 16;
            entries[i] = .{
                .key = std.mem.readInt(u64, buf[off..][0..8], .big),
                .raw_move = std.mem.readInt(u16, buf[off + 8 ..][0..2], .big),
                .weight = std.mem.readInt(u16, buf[off + 10 ..][0..2], .big),
                // 4 bytes of learn data ignored
            };
        }
    ...
    // Load book from file path.
    // IO: read all bytes from file at `path` into a buffer, obtain a random u64 seed,
    // then call fromBytes(allocator, buf, seed). Free the temp buffer afterwards.
    pub fn load(io: std.Io, allocator: std.mem.Allocator, path: []const u8) !Book {
        ...
        if (@import("builtin").os.tag == .linux) {
            ...
            const book = try fromBytes(allocator, buf, seed);
            return book;
        } else {
            ...
            const book = try fromBytes(allocator, buf[0..len], seed);
            return book;
        }
    }
    ...
    pub fn probe(self: *Book, state: *const State) ?Move {
        const key = polyglotHash(state);
        ...
        // Collect all entries with matching key, sum weights
        var total_weight: u32 = 0;
        var end_idx: usize = lo;
        while (end_idx < self.entries.len and self.entries[end_idx].key == key) {
            total_weight += self.entries[end_idx].weight;
            end_idx += 1;
        }

        if (lo == end_idx) return null;

        // Weighted random selection (or first entry if all weights are 0)
        var selected = lo;
        if (total_weight > 0) {
            var target = self.rng.random().int(u32) % total_weight;
            for (lo..end_idx) |i| {
                if (target < self.entries[i].weight) {
                    selected = i;
                    break;
                }
                target -= self.entries[i].weight;
            }
        }

        const move = decodeMove(self.entries[selected].raw_move, state) orelse return null;

        // Validate against legal moves
        const legal = movegen.legalMoves(state, state.to_move);
        for (0..legal.len) |i| {
            if (legal.moves[i].eql(move)) return move;
        }
...
// Compute Polyglot Zobrist hash from engine State
pub fn polyglotHash(state: *const State) u64 {
    var hash: u64 = 0;

    // Piece-square keys
    for (0..64) |sq_idx| {
        const sq: Square = @intCast(sq_idx);
        if (state.mailbox[sq]) |p| {
            const color = state.colorAt(sq).?;
            // Polyglot piece index: kind * 2 + pg_color
            // pg_color: black=0, white=1 (opposite of engine's Color where white=0, black=1)
            const p_idx: usize = @intCast(p);
            const pg_color: usize = @intCast(~color);
            const pg_piece_idx = p_idx * 2 + pg_color;
            hash ^= polyglot_random[pg_piece_idx * 64 + sq_idx];
        }
    }
    ...
    if (state.castling_rights & castling.white_queenside != 0) hash ^= polyglot_random[769];
    if (state.castling_rights & castling.black_kingside != 0) hash ^= polyglot_random[770];
    if (state.castling_rights & castling.black_queenside != 0) hash ^= polyglot_random[771];

    // En passant: only hash if a pawn of the side to move can actually capture
    if (state.en_passant) |ep| {
        const ep_file: usize = @intCast(ep % 8);
        const ep_rank: usize = @intCast(ep / 8);
        // Rank of pawns that could capture en passant
        const pawn_rank: usize = if (state.to_move == Colors.white) ep_rank - 1 else ep_rank + 1;
        var has_ep_pawn = false;
        if (ep_file > 0) {
            const adj_sq: Square = @intCast(pawn_rank * 8 + ep_file - 1);
            if (state.mailbox[adj_sq] == piece.pawn) {
                if (state.colorAt(adj_sq) == state.to_move) has_ep_pawn = true;
            }
        }
        if (ep_file < 7) {
            const adj_sq: Square = @intCast(pawn_rank * 8 + ep_file + 1);
        ...
        if (has_ep_pawn) hash ^= polyglot_random[772 + ep_file];
    }

    // Turn: XOR if white to move
    if (state.to_move == Colors.white) hash ^= polyglot_random[780];

    return hash;
}

// Decode Polyglot 16-bit move encoding to engine Move.
// Bits: to_file[0:2] to_row[3:5] from_file[6:8] from_row[9:11] promotion[12:14]
fn decodeMove(raw: u16, state: *const State) ?Move {
    const to_file: u3 = @intCast(raw & 0x7);
    const to_row: u3 = @intCast((raw >> 3) & 0x7);
    const from_file: u3 = @intCast((raw >> 6) & 0x7);
    const from_row: u3 = @intCast((raw >> 9) & 0x7);
    const promotion: u3 = @intCast((raw >> 12) & 0x7);

    const from_sq: Square = @as(Square, from_row) * 8 + from_file;
    var to_sq: Square = @as(Square, to_row) * 8 + to_file;

    // Castling: Polyglot encodes as "king captures own rook", engine uses king destination
    if (state.mailbox[from_sq] == piece.king) {
        if (from_sq == square.e1 and to_sq == square.h1) {
            to_sq = square.g1; // White O-O
        } else if (from_sq == square.e1 and to_sq == square.a1) {
            to_sq = square.c1; // White O-O-O
        } else if (from_sq == square.e8 and to_sq == square.h8) {
            to_sq = square.g8; // Black O-O
        } else if (from_sq == square.e8 and to_sq == square.a8) {
            to_sq = square.c8; // Black O-O-O
        }
    }

    if (promotion != 0) {
        // Polyglot: 1=knight, 2=bishop, 3=rook, 4=queen
        // Engine:   knight=1, bishop=2, rook=3, queen=4 (same mapping)
        const promo_piece: piece.Piece = switch (promotion) {
            1 => piece.knight,
            2 => piece.bishop,
            3 => piece.rook,
            4 => piece.queen,
...
// Known Polyglot hash test vectors
test "polyglot hash - starting position" {
    const state = State.defaultPosition();
    try expectEqual(@as(u64, 0x463b96181691fc9c), polyglotHash(&state));
}

test "polyglot hash - after 1.e4" {
    var state = State.defaultPosition();
    _ = state.makeMove(.{ .start = square.e2, .end = square.e4 }, Colors.white, piece.pawn);
    try expectEqual(@as(u64, 0x823c9b50fd114196), polyglotHash(&state));
}

test "polyglot hash - after 1.e4 d5" {
    var state = State.defaultPosition();
    _ = state.makeMove(.{ .start = square.e2, .end = square.e4 }, Colors.white, piece.pawn);
    _ = state.makeMove(.{ .start = square.d7, .end = square.d5 }, Colors.black, piece.pawn);
    try expectEqual(@as(u64, 0x0756b94461c50fb0), polyglotHash(&state));
}

test "polyglot hash - after 1.e4 d5 2.e5" {
    var state = State.defaultPosition();
    _ = state.makeMove(.{ .start = square.e2, .end = square.e4 }, Colors.white, piece.pawn);
    _ = state.makeMove(.{ .start = square.d7, .end = square.d5 }, Colors.black, piece.pawn);
    _ = state.makeMove(.{ .start = square.e4, .end = square.e5 }, Colors.white, piece.pawn);
    try expectEqual(@as(u64, 0x662fafb965db29d4), polyglotHash(&state));
}

test "polyglot hash - after 1.e4 d5 2.e5 f5 (en passant)" {
    var state = State.defaultPosition();
    _ = state.makeMove(.{ .start = square.e2, .end = square.e4 }, Colors.white, piece.pawn);
    _ = state.makeMove(.{ .start = square.d7, .end = square.d5 }, Colors.black, piece.pawn);
    _ = state.makeMove(.{ .start = square.e4, .end = square.e5 }, Colors.white, piece.pawn);
    _ = state.makeMove(.{ .start = square.f7, .end = square.f5 }, Colors.black, piece.pawn);
    try expectEqual(@as(u64, 0x22a48b5a8e47ff78), polyglotHash(&state));
}

test "decode move - regular move" {
    const state = State.defaultPosition();
    // e2e4: from_file=4, from_row=1, to_file=4, to_row=3
    const raw: u16 = (1 << 9) | (4 << 6) | (3 << 3) | 4;
    const move = decodeMove(raw, &state).?;
    try expectEqual(square.e2, move.start);
    try expectEqual(square.e4, move.end);
    try expect(!move.is_promotion);
}

test "decode move - castling remapped" {
    const state = State.fromFen("r3k2r/pppppppp/8/8/8/8/PPPPPPPP/R3K2R w KQkq - 0 1") catch unreachable;
    // Polyglot O-O: e1h1 (king captures rook) → engine e1g1
    const raw: u16 = (0 << 9) | (4 << 6) | (0 << 3) | 7;
    const move = decodeMove(raw, &state).?;
    try expectEqual(square.e1, move.start);
    try expectEqual(square.g1, move.end);
}

test "decode move - promotion" {
    const state = State.fromFen("8/P7/8/8/8/8/8/4K2k w - - 0 1") catch unreachable;
    // a7a8q: from_file=0, from_row=6, to_file=0, to_row=7, promotion=4(queen)
    const raw: u16 = (4 << 12) | (6 << 9) | (0 << 6) | (7 << 3) | 0;
    const move = decodeMove(raw, &state).?;
    try expectEqual(square.a7, move.start);
    try expectEqual(square.a8, move.end);
    try expect(move.is_promotion);
    try expectEqual(piece.queen, move.promotion_piece);
}
